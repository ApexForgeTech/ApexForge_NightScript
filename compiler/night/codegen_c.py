...
class CGenerator:
    ...
    def _emit_struct_definitions(self) -> list[str]:
        ...
        for decl in self.check_result.program.declarations:
            ...
            for field in decl.fields:
                field_type = self._c_type(self.check_result.semantic_model.structs[decl.name].type_ref.fields[
                    next(i for i, candidate in enumerate(self.check_result.semantic_model.structs[decl.name].type_ref.fields) if candidate.name == field.name)
                ].type_ref)
    ...
    def _emit_expr(self, expr: ast.Expr) -> str:
        ...
        if isinstance(expr, ast.FieldAccessExpr):
            return f"{self._emit_expr(expr.object)}.{expr.field}"
    ...
    def _format_function_signature(self, decl: ast.FunctionDecl | ast.ExternFunctionDecl) -> str:
        signature = self.check_result.type_result.functions[decl.name]
        ...
        return f"{return_type} {decl.name}({params_text})"
